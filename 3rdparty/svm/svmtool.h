...
/*This function is used for shape-sampling*/
/* data:        true represents material
 * flag	:       true represents the sampling area
 * width:       image width
 * height:		image height
 * area_low:	lower limit of the sample area
 * area_up:		upper limit of the sample area
 * type:        svm type
 * out_features:	out features value
 * out_length:		out features length
 * out_flag:	mark the selected sample*/
void shape_sampling(const bool* data, const bool* flag, int width, int height,
    int area_low, int area_up, svm_type type, int* out_features, int& out_length, bool* out_flag);

/*This function is used to classify by svm shape*/
/* data:        true represents material
 * width:       image width
 * height:		image height
 * area_low:	lower limit of the sample area
 * area_up:		upper limit of the sample area
 * type:        svm type
 * out_flag:	mark the bad sample*/
void shape_classify(const bool* data, const double coefs[COEF_COUNT], int width, int height,
    int area_low, int area_up, svm_type type, bool* out_flag);
