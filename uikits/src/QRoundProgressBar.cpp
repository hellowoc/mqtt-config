    ...
    m_updateFlags(UF_PERCENT)
{
    ...
    add_btn->setAutoRepeat(true);
    add_btn->setAutoRepeatDelay(300);

    sub_btn = new QPushButton(this);
    sub_btn->setFocusPolicy(Qt::NoFocus);
    sub_btn->setObjectName("subBtn");
    sub_btn->setAutoRepeat(true);
    sub_btn->setAutoRepeatDelay(300);
    ...
    connect(sub_btn,SIGNAL(clicked(bool)),this,SLOT(on_sub_btn_clicked(bool)));
}
...
void QRoundProgressBar::setRange(double min, double max)
{
    ...
    update();
    int x = geometry().x();
    add_btn->move(width()-54-10,width()-10);
    sub_btn->move(10,width()-10);
}
...
void QRoundProgressBar::on_add_btn_clicked(bool click)
{
...
void QRoundProgressBar::on_sub_btn_clicked(bool click)
{
     int tmpValue = m_value;
    if(tmpValue > m_min)
        tmpValue-=1;
...
//        p.setPen(QPen(palette().shadow().color(), m_outlinePenWidth));
        p.setPen(QPen(QColor(Qt::white), m_outlinePenWidth));
                                  ...
                                  , double innerRadius)
{
    ...
    {
        p.setPen(QPen(QColor(51,153,255,180), m_dataPenWidth));
...
//    p.setBrush(QBrush(QColor(240,240,30)));
 	  p.setBrush(QBrush(QColor(30,232,216)));
...
//    p.setPen(QColor(200,220,10));
      p.setPen(QColor(30,232,216));
...
void QRoundProgressBar::setButtonStyle(QString style)
{
    add_btn->show();
    sub_btn->show();
}
