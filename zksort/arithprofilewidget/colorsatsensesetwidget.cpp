#include "colorsatsensesetwidget.h"
#include "ui_colorsatsensesetwidget.h"
...
ColorSatSenseSetWidget::ColorSatSenseSetWidget(QWidget *parent) :
    basewidget(parent),
    ui(new Ui::ColorSatSenseSetWidget)
{
    ui->setupUi(this);
...
ColorSatSenseSetWidget::~ColorSatSenseSetWidget()
{
    delete ui;
}

void ColorSatSenseSetWidget::showPage(bool bshow)
{
    if(bshow){
...
void ColorSatSenseSetWidget::receiveMsg(int msg1, int msg2, int msg3)
{
    ...
    if(msg1 == MSG_DATA_CHANGED){
        if(msg2 < MAX_VIEW_UNIT_USE*2){
            ...
            switch (msg2) {
            case 40:
                struCnfp.struGroupIdentify[struGsh.nLayer][struGsh.nView][struGsh.nMatGroup].struColorSat.nRow = m_rowInt;
                if(m_percentInt > m_rowInt*m_rowInt)
                {
                    m_percentInt = m_rowInt*m_rowInt;
                    struCnfp.struGroupIdentify[struGsh.nLayer][struGsh.nView][struGsh.nMatGroup].struColorSat.nPercent = m_percentInt;
                }
            ...
            case 41:
                ...
                break;
            case 42:
                struCnfp.struGroupIdentify[struGsh.nLayer][struGsh.nView][struGsh.nMatGroup].struColorSat.lightLimit = m_lightLimit;
                break;
            default:
                break;
            }
...
void ColorSatSenseSetWidget::retranslateUiWidget()
{
...
//    ui->label_13->setText(g_myLan().sens_update_enable);
...
void ColorSatSenseSetWidget::updateValue()//更新保留算法相关的数据
{
}

void ColorSatSenseSetWidget::updateChuteState()//更新料槽的配置精度值
{
    ...
    QByteArray titleArr(groupIde.struColorSat.sName);
    ui->sNamelineEdit->setText(QString::fromUtf8(titleArr));

    ui->sNamelineEdit->setType(textType);
    int standardSens;
    if(groupIde.struColorSat.nMode){
        standardSens = groupIde.struColorSat.nSensUp;

    }else{
        standardSens = groupIde.struColorSat.nSensLow;
    }
    ...
    for(int i = 0;i < tempRepeatCondition;i++){
        ...
        if(struCnfp.nMatCopySetMode == MAT_COPY_ALL_SAME || struCnfp.nMatCopySetMode == MAT_COPY_REAR_ALIKE_FRONT){//前后视相同
            ...
            if(groupInfo.nUnitGupSplice[i] == 2){
                m_pChuteSenseEdit[0][unitIndex]->setMinMax(this,0-stand,ARITH_SAT_MAX_SENSE-stand,-1,&struCnfp.struUnitSens[struGsh.nLayer][struGsh.nView][unitIndex][1].sat);
                m_pChuteSenseEdit[1][unitIndex]->setMinMax(this,0-stand,ARITH_SAT_MAX_SENSE-stand,-1,&struCnfp.struUnitSens[struGsh.nLayer][struGsh.nView+1][unitIndex][1].sat);
                m_pChuteSenseEdit[0][unitIndex]->setText(IntToQString(struCnfp.struUnitSens[struGsh.nLayer][struGsh.nView][unitIndex][1].sat));
                m_pChuteSenseEdit[1][unitIndex]->setText(IntToQString(struCnfp.struUnitSens[struGsh.nLayer][struGsh.nView+1][unitIndex][1].sat));
            }else{
                m_pChuteSenseEdit[0][unitIndex]->setMinMax(this,0-stand,ARITH_SAT_MAX_SENSE-stand,-1,&struCnfp.struUnitSens[struGsh.nLayer][struGsh.nView][unitIndex][0].sat);
                m_pChuteSenseEdit[1][unitIndex]->setMinMax(this,0-stand,ARITH_SAT_MAX_SENSE-stand,-1,&struCnfp.struUnitSens[struGsh.nLayer][struGsh.nView+1][unitIndex][0].sat);
                m_pChuteSenseEdit[0][unitIndex]->setText(IntToQString(struCnfp.struUnitSens[struGsh.nLayer][struGsh.nView][unitIndex][0].sat));
                m_pChuteSenseEdit[1][unitIndex]->setText(IntToQString(struCnfp.struUnitSens[struGsh.nLayer][struGsh.nView+1][unitIndex][0].sat));
            }
        }else{
            if(struGsh.nView%2){
                ui->label_front->hide();
            ...
            if(groupInfo.nUnitGupSplice[i] == 2){
                m_pChuteSenseEdit[struGsh.nView%2][unitIndex]->setMinMax(this,0-stand,ARITH_SAT_MAX_SENSE-stand,-1,&struCnfp.struUnitSens[struGsh.nLayer][struGsh.nView][unitIndex][1].sat);
                m_pChuteSenseEdit[struGsh.nView%2][unitIndex]->setText(IntToQString(struCnfp.struUnitSens[struGsh.nLayer][struGsh.nView][unitIndex][1].sat));
            }else{
                m_pChuteSenseEdit[struGsh.nView%2][unitIndex]->setMinMax(this,0-stand,ARITH_SAT_MAX_SENSE-stand,-1,&struCnfp.struUnitSens[struGsh.nLayer][struGsh.nView][unitIndex][0].sat);
                m_pChuteSenseEdit[struGsh.nView%2][unitIndex]->setText(IntToQString(struCnfp.struUnitSens[struGsh.nLayer][struGsh.nView][unitIndex][0].sat));
            }
...
void ColorSatSenseSetWidget::updateEdgeBtn()
{

}

void ColorSatSenseSetWidget::updateinfectBtn()
{

}

void ColorSatSenseSetWidget::setViewVisibleByArith()//更新保留算法控件的显示和隐藏
{
...
void ColorSatSenseSetWidget::updateModeBtnState()
{
    if(struCnfp.struGroupIdentify[struGsh.nLayer][struGsh.nView][struGsh.nMatGroup].struColorSat.nMode == 0){
        m_pModeBtnGroup->button(0)->setChecked(true);
    }else{
        m_pModeBtnGroup->button(1)->setChecked(true);
    }
}
void ColorSatSenseSetWidget::onModeChange(int mode)
{
    struCnfp.struGroupIdentify[struGsh.nLayer][struGsh.nView][struGsh.nMatGroup].struColorSat.nMode = mode;
}

void ColorSatSenseSetWidget::updatePercentViewState()//更新比例算法相关控件的显示和隐藏
{
  //version 1.5
...
void ColorSatSenseSetWidget::updateLineDefectState()
{
    ui->lineDefectCheckbox->hide();
    ui->lineDefectGroupbox->hide();
}

void ColorSatSenseSetWidget::updateSizeState()
{
    int layer = struGsh.nLayer;
    int view = struGsh.nView;
    int group = struGsh.nMatGroup;
    int index = struGsh.nArithIndex;
    stru_color_sat  &color_sat  = struCnfp.struGroupIdentify[layer][view][group].struColorSat;
    int maxPercent = 0;
    maxPercent = color_sat.nRow*color_sat.nRow;
    m_rowInt = color_sat.nRow;
    m_percentInt = color_sat.nPercent;
    m_lightLimit = color_sat.lightLimit;
    ...
    ui->lightEdit->setMinMax(this,1,255,42,&m_lightLimit);
}

void ColorSatSenseSetWidget::resRadioBtnClicked(bool checked)
{

}

void ColorSatSenseSetWidget::on_edgeBtn_clicked(bool checked)
{

}

void ColorSatSenseSetWidget::on_infectBtn_clicked(bool checked)
{

}

void ColorSatSenseSetWidget::on_comboBox_activated(int index)
{

}

void ColorSatSenseSetWidget::sNameChanged()
{
    memset(struCnfp.struGroupIdentify[struGsh.nLayer][struGsh.nView][struGsh.nMatGroup].struColorSat.sName,0,sizeof(struCnfp.struGroupIdentify[struGsh.nLayer][struGsh.nView][struGsh.nMatGroup].struColorSat.sName));

    QString name = ui->sNamelineEdit->text();
    QByteArray namearr = name.toUtf8();
    const char *strName = namearr.data();
    memcpy(struCnfp.struGroupIdentify[struGsh.nLayer][struGsh.nView][struGsh.nMatGroup].struColorSat.sName,strName,strlen(strName));
}

void ColorSatSenseSetWidget::on_SensUpdateCheckBox_clicked(bool checked)
{
...
void ColorSatSenseSetWidget::on_comboBox_diff_activated(int index)
{

}

void ColorSatSenseSetWidget::on_comboBox_gray_activated(int index)
{

}

void ColorSatSenseSetWidget::modeBtnClicked(bool check)//算法的选亮选暗切换
{
    if(check)
...
void ColorSatSenseSetWidget::modeBtnRes1Clicked(bool)//灰度保留的选亮选暗切换
{
...
void ColorSatSenseSetWidget::modeBtnRes2Clicked(bool)//色差保留的选亮选暗切换
{
...
void ColorSatSenseSetWidget::on_percentCheckBox_clicked(bool checked)
{

}

void ColorSatSenseSetWidget::on_pushButton_3_clicked(bool checked)
{
...
void ColorSatSenseSetWidget::on_lineDefectCheckbox_clicked(bool checked)
{
