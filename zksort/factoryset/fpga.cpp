...
fpga::fpga(QWidget *parent)
    : QDialog(parent)
{
    setGeometry(0, 0, LCD_WIDTH,  struCnfg.screenH-LCD_TITLE_HEIGHT);
...
void fpga::initUpdateParams()
{
    ...
    {
        defCameraMode[i] = struCnfp.nCameraBoardMode[0][i];
        cameraUserMode[i] = 0;
    }
...
//    warningLabel->setFont(config->getFont(DEFAULT_FONT_SIZE));
    warningLabel->setFixedHeight( struCnfg.screenH/3);
    warningLabel->setAlignment(Qt::AlignCenter);

    QSize btnSize(BTN_WIDTH, BTN_HEIGHT);
//    mountUdiskBackBtn = new myPushButton(g_myLan().upgrade_return, ":/white/icon_back.png",QRect());
    mountUdiskBackBtn = new myPushButton("返回/Back", ":/white/icon_back.png",QRect());
    mountUdiskBackBtn->setFixedSize(btnSize);
//    mountUdiskGoBtn = new myPushButton(g_myLan().next,":/white/icon_right.png",QRect());
...
void fpga::createFileListPage()
{
    ...
    fileList->setMinimumHeight( struCnfg.screenH/2+50);
...
//        fileItem[i]->setFont(config->getFont(DEFAULT_FONT_SIZE));
    ...
    upLay->setContentsMargins(LCD_WIDTH/5,  struCnfg.screenH/10, LCD_WIDTH/5, 20);
...
void fpga::createNoticePage()
{
    ...
    noticeLabel = new myLabel(g_myLan().upgrade, QRect());
    noticeLabel->setAlignment(Qt::AlignCenter);
    noticeLabel->setFixedHeight(100);

    QSize btnSize(BTN_WIDTH, BTN_HEIGHT);
    noticeLastBtn = new myPushButton(g_myLan().last, ":/white/icon_left.png", QRect());
    noticeLastBtn->setFixedSize(btnSize);
    noticeNextBtn = new myPushButton(g_myLan().next, ":/white/icon_right.png", QRect());
...
/*
    if (struCnfe.nProduct == OPT_PRODUCT_ANCOO_MAIZE) {
        ...
        layerMaizeAddGroup->setLabelText(g_myLan().lower_view,g_myLan().upper_view);
        layerMaizeAddGroup->setCurrentIndex(0);
        layerMaizeAddGroup->show();
    } else if (struCnfe.nMachine == MACHINE_LD4) {
        layerMaizeAddGroup->setNum(2);
        layerMaizeAddGroup->setLabelText(g_myLan().master_view,g_myLan().slave_view);
        layerMaizeAddGroup->setCurrentIndex(0);
        layerMaizeAddGroup->show();
    } else if (struCnfe.nMachine == MACHINE_LDS2) {
        layerMaizeAddGroup->setNum(2);
        layerMaizeAddGroup->setLabelText(g_myLan().ids_frontCamera,g_myLan().ids_backCamera);
        layerMaizeAddGroup->setCurrentIndex(0);
        layerMaizeAddGroup->show();
    } else {
...
//    boardNumGroup->setFont(config->getFont());
    ...
    messagesGroup->setFixedHeight( struCnfg.screenH-LCD_TITLE_HEIGHT-LCD_STATUS_HEIGHT-180);
...
void fpga::createWholeUpdatePage()
{
    ...
    group->setFixedHeight( struCnfg.screenH-LCD_TITLE_HEIGHT-LCD_STATUS_HEIGHT-80);
...
//            layerGroup->show();
//            layerGroup->setNum(2);
//            layerGroup->setLabelText(g_myLan().upper_layer, g_myLan().lower_layer);
//            layerGroup->setTitle(g_myLan().layer);
//        } else if(struCnfe.nMachine==MACHINE_TD && struCnfg.nLevelTotal==3){
//            layerGroup->show();
//            layerGroup->setNum(3);
//            layerGroup->setLabelText(g_myLan().upper_layer, g_myLan().mid_layer, g_myLan().lower_layer);
//            layerGroup->setTitle(g_myLan().layer);
//        } else if (struCnfe.nMachine==MACHINE_LD4) {
//            layerGroup->hide();
//        } else if (struCnfe.nMachine==MACHINE_LDS2) {
...
//        layerGroup->hide();
//        num = 4;
//        updateMesLabels[0]->setText(QString("1.%1").arg(g_myLan().msg_int_to_factory));
//        updateMesLabels[1]->setText(QString("2.%1").arg(g_myLan().msg_int_download_file));
//        updateMesLabels[2]->setText(QString("3.%1").arg(g_myLan().msg_int_burn_file));
//        updateMesLabels[3]->setText(QString("4.%1").arg(g_myLan().msg_int_to_user));

//        for(int i=0; i<num; i++){
//            updateStatusLabels[i]->setText(g_myLan().upgrade_untreated);
//        }
...
//        break;
    ...
    case COLOR_BOARD:      
        ...
        layerGroup->setNum(2);
        layerGroup->setLabelText("ids_frontCamera", "ids_backCamera");
...
void fpga::setUpdateWholeDisplay()
{
    ...
    for(int i=0; i<MAX_VIEW; i++){
        if(curBoardMode == COLOR_BOARD){
            boardNumLabels[0][i]->setText(QString("%1 %2 %3").arg("ids_frontCamera").arg(i+1).arg("upgrade"));
            boardNumLabels[1][i]->setText(QString("%1 %2 %3").arg("ids_backCamera").arg(i+1).arg("upgrade"));
...
//        if(stackedWidget->currentIndex() == UPDATE_PAGE){
//            per = (100*i)/writeLen;
//            QString str = QString("%1 %2%").arg(g_myLan().upgrade_doing).arg(per);
//            updateStatusLabels[1]->setText(str);
//        }
...
bool fpga::ctrlSendFile()
{
    ...
    for(int i=0; i<writeLen; i++){
        if(stackedWidget->currentIndex() == UPDATE_PAGE){
            ...
            per = (100*i)/writeLen;
            QString str = QString("%1 %2%").arg(g_myLan().upgrade).arg(per);
...
//    }
    ...
    if(bChangeBoardMode){
        struCnfp.nCameraBoardMode[curLevel][curView] = cameraUserMode[curView];
    }
...
//    if(bTransByUSB)
//        transModeBtn->setText(g_myLan().ai_high_speed);
//    else
//        transModeBtn->setText(g_myLan().ai_low_speed);
...
void fpga::onBoardUpdateRetryBtnClicked()
{
    ...
    if(curUpdateStep == 0){
        ...
        case INT_BOARD:
            ...
            break;
        case COLOR_BOARD:
            struCnfp.nCameraBoardMode[0][curView] = cameraUserMode[curView];
            break;
        default:
            break;
        }
...
//    switch(index){
//    case MOUNT_PAGE:
//        titleLabel->setText(QString("%1 > %2").arg(g_myLan().ids_set).arg(g_myLan().upgrade_fpga));
//        break;
//    case FILE_LIST_PAGE:
//        titleLabel->setText(QString("%1 > %2 > %3").arg(g_myLan().ids_set).arg(g_myLan().upgrade_fpga)
//                            .arg(g_myLan().upgrade_file_list));
//        break;
//    case NOTICE_PAGE:
//        titleLabel->setText(QString("%1 > %2 > %3").arg(g_myLan().ids_set).arg(g_myLan().upgrade_fpga)
//                            .arg(g_myLan().upgrade_usb_communication));
//        break;
//    case UPDATE_PAGE:
//        titleLabel->setText(QString("%1 > %2 > %3").arg(g_myLan().ids_set).arg(g_myLan().upgrade_fpga)
//                            .arg(g_myLan().upgrade_board));
//        break;
//    case WHOLE_UPDATE_PAGE:
//        titleLabel->setText(QString("%1 > %2 > %3").arg(g_myLan().ids_set).arg(g_myLan().upgrade_fpga)
//                            .arg(g_myLan().upgrade_whole));
//        break;
//    default:
//        break;
//    }
