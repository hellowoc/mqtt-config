    ...
    ui(new Ui::SetEditValueWidget)
{
    ...
    ui->label_2->hide();
    ui->label_3->hide();
    ui->subBtn->setAutoRepeat(true);
    ui->subBtn->setAutoRepeatDelay(500);
    ui->subBtn->setAutoRepeatInterval(100);

    ui->addBtn->setAutoRepeat(true);
    ui->addBtn->setAutoRepeatDelay(500);
    ui->addBtn->setAutoRepeatInterval(100);}
...
void SetEditValueWidget::setFloatParams(AbstractInterface *parent, float min, float max, int index, float *valueAddr)
{
    m_type = floatType;
    m_pParent = parent;
    m_maxFloat = max;
...
{
    m_valueFloat = value;
    ui->lineEdit->setText(QString::number(value,'f',1));
}
...
void SetEditValueWidget::on_subBtn_clicked()
{
    if(m_type == IntType){
        ...
        if(m_valueFloat - m_minFloat > 0.00001){
            m_valueFloat-=0.1;
...
void SetEditValueWidget::on_addBtn_clicked()
{
    if(m_type == IntType){
